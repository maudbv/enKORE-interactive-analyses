...
ui <- pageWithSidebar(
  ...
  sidebarPanel(
    selectInput('hyp', 'Select a hypothesis', c('limiting similarity','Darwin naturalisation',
                                                'Phenotypic plasticity','Disturbance','Propagule pressure',
                                                'Island susceptibility','Invasional meltdown',
                                                'Tens rule','Biotic resistence','Enemy release')),
  ),
  mainPanel(
    #textOutput("selected"),
...
limiting <- read.csv(file = 'files/limiting_1.csv')
limiting <- limiting[!duplicated(limiting$Title),]

darwin <- read.csv(file = 'files/darwin_22.csv')
darwin <- darwin[!duplicated(darwin$Title),]

phynotypic <- read.csv(file = 'files/phynotypic_3.csv')
phynotypic <- phynotypic[!duplicated(phynotypic$Title),]

disturbance <- read.csv(file = 'files/disturbance_44.csv')
disturbance <- disturbance[!duplicated(disturbance$Title),]

pressure <- read.csv(file = 'files/pressure_55.csv')
pressure <- pressure[!duplicated(pressure$Title),]

island <- read.csv(file = 'files/island_66.csv')
island <- island[!duplicated(island$Title),]

meltdown <- read.csv(file = 'files/meltdown_77.csv')
meltdown <- meltdown[!duplicated(meltdown$Title),]

tens <- read.csv(file = 'files/tens_88.csv')
tens <- tens[!duplicated(tens$Title),]

biotic <- read.csv(file = 'files/biotic_99.csv')
biotic <- biotic[!duplicated(biotic$Title),]

enemy <- read.csv(file = 'files/enemy_1010.csv')
enemy <- enemy[!duplicated(enemy$Title),]



df_list <- list(limiting,darwin,phynotypic,disturbance,pressure,island,
                meltdown,tens,biotic,enemy)
total <- Reduce(
